...
if ( ! function_exists( 'gulir_series_page_header' ) ) {
	function gulir_series_page_header( $settings = [] ) {

		if ( empty( $settings['category_header'] ) ) {
			gulir_series_header_1( $settings );
		...
		switch ( $settings['category_header'] ) {
			case '2' :
				gulir_series_header_2( $settings );
				break;
			case '3' :
...
if ( ! function_exists( 'gulir_podcast_single_breadcrumb' ) ) {
	function gulir_podcast_single_breadcrumb() {

		if ( gulir_get_option( 'single_podcast_breadcrumb' ) ) {
			echo gulir_get_breadcrumb( 's-breadcrumb' );
		}
	}
}

if ( ! function_exists( 'gulir_podcast_icon' ) ) {
	function gulir_podcast_icon() {

		if ( 'podcast' !== get_post_type() ) {
			return;
		}

		$icon = gulir_get_option( 'podcast_custom_icon' );
		if ( ! empty( $icon['url'] ) ) {
...
if ( ! function_exists( 'gulir_podcast_socials_overlay' ) ) {
	function gulir_podcast_socials_overlay() {

		if ( 'podcast' !== get_post_type() || ! gulir_get_option( 'podcast_socials_overlay' ) ) {
			return;
		}
		$socials = gulir_get_listen_on_settings();
		if ( ! count( $socials ) ) {
			return;
		}
		$index = 1;
		...
		<div class="podcast-social-overlay meta-bold">
			<?php foreach ( $socials as $social ) :
				...
				?>
			<a href="<?php echo esc_url( $social['url'] ); ?>" data-gravity="s" data-title="<?php echo gulir_attr__( 'Listen on', 'gulir' ) . ' ' . strip_tags( $social['label'] ); ?>" class="podcast-social-item" target="_blank" rel="nofollow">
				<i class="<?php echo strip_tags( $icon_classes ); ?>" aria-hidden="true"></i><span><?php gulir_render_inline_html( $social['label'] ); ?></span>
				</a><?php
				if ( $index > 2 ) {
					break;
				}
...
if ( ! function_exists( 'gulir_podcast_socials' ) ) {
	function gulir_podcast_socials() {
		...
		<div class="podcast-socials">
			<div class="podcast-social-label is-meta">
				<i class="rbi rbi-frequency"></i><span><?php gulir_html_e( 'Listen on', 'gulir' ); ?></span></div>
			<div class="podcast-social-list">
				<?php foreach ( $socials as $social ) :
					...
					?>
					<a href="<?php echo esc_url( $social['url'] ); ?>" data-gravity="s" data-title="<?php echo gulir_attr__( 'Listen on', 'gulir' ) . ' ' . strip_tags( $social['label'] ); ?>" class="<?php echo 'podcast-social-item social-' . trim( $social['icon'] ); ?>" target="_blank" rel="nofollow">
						<i class="<?php echo strip_tags( $icon_classes ); ?>"></i><span><?php gulir_render_inline_html( $social['label'] ); ?></span>
					</a>
				<?php endforeach; ?>
			</div>
		</div>
...
if ( ! function_exists( 'gulir_podcast_entry_meta_play' ) ) {
	function gulir_podcast_entry_meta_play( $settings ) {
		...
		<?php if ( ! empty( $settings['has_play_label'] ) ): ?>
			<span class="meta-play-label" data-pause="<?php gulir_attr_e( 'PAUSE', 'gulir' ); ?>" data-listen="<?php gulir_attr_e( 'LISTEN', 'gulir' ); ?>"><?php echo gulir_html__( 'LISTEN', 'gulir' ); ?></span>
		<?php endif; ?>
		</span>
		<?php
	}
...
if ( ! function_exists( 'gulir_podcast_entry_player' ) ) {
	function gulir_podcast_entry_player( $settings ) {

		if ( 'podcast' !== get_post_type() ) {
			return;
		} ?>
		<div class="entry-player"><?php echo gulir_get_audio_embed(); ?></div>
		<?php
	}
}

if ( ! function_exists( 'gulir_podcast_featured' ) ) {
	/**
	 * @param array $settings
	 * render featured image
	 */
	function gulir_podcast_featured( $settings = [] ) {
...
if ( ! function_exists( 'gulir_podcast_featured_only' ) ) {
	/**
	 * @param array $settings
	 */
	function gulir_podcast_featured_only( $settings = [] ) {

		if ( ! empty( $settings['crop_size'] ) && gulir_has_featured_image( $settings['crop_size'] ) ) : ?>
			<div class="feat-holder"><?php gulir_podcast_featured( $settings ); ?></div>
		<?php endif;
	}
}

if ( ! function_exists( 'gulir_podcast_featured_with_category' ) ) {
	/**
	 * @param array $settings
	 */
	function gulir_podcast_featured_with_category( $settings = [] ) {

		if ( ! empty( $settings['crop_size'] ) && gulir_has_featured_image( $settings['crop_size'] ) ) : ?>
			<div class="feat-holder">
				<?php
				gulir_podcast_featured( $settings );
...
if ( ! function_exists( 'gulir_podcast_title' ) ) {
	/**
	 * @param $settings
	 */
	function gulir_podcast_title( $settings = [] ) {
		...
		$classes    = apply_filters( 'gulir_entry_title_classes', $classes, get_the_ID() );
...
if ( ! function_exists( 'gulir_podcast_entry_meta' ) ) {
	function gulir_podcast_entry_meta( $settings = [] ) {

		$settings['meta_label_by']      = gulir_get_option( 'podcast_meta_author_label' );
		$settings['has_duration_label'] = gulir_get_option( 'podcast_meta_duration_label' );
		$settings['has_play_label']     = gulir_get_option( 'podcast_meta_play_label' );
...
if ( ! function_exists( 'gulir_series_header_1' ) ) {
	function gulir_series_header_1( $settings = [] ) {
				...
				<div class="archive-inner">
					...
					<?php if ( ! empty( $settings['featured_image'] ) ) :
						...
						<div class="category-hero-wrap">
							<?php gulir_render_category_hero( $settings['featured_image'], $settings['featured_image_urls'] );
							if ( ! empty( $settings['featured_file'][0] ) ) : ?>
								<div class="series-intro series-intro-absolute">
									<?php echo gulir_get_audio_hosted( [
...
if ( ! function_exists( 'gulir_series_header_2' ) ) {
	/**
	 * @param array $settings
	 */
	function gulir_series_header_2( $settings = [] ) {
...
if ( ! function_exists( 'gulir_series_socials' ) ) {
	function gulir_series_socials( $settings = [] ) {
		...
		if ( gulir_get_option( 'series_rss' ) ) {
			$socials[] = [
				'label' => 'Subscribe',
				'icon'  => 'rbi-rss',
...
if ( ! function_exists( 'gulir_podcast_blog' ) ) {
	/**
	 * @param array $settings
	 * @param null  $_query
	 */
	function gulir_podcast_blog( $settings = [], $_query = null ) {
			...
			<div class="rb-container edge-padding">
				<div class="grid-container">
					<div class="blog-content">
						<?php echo gulir_get_podcast_grid_flex_1( $settings, $_query ); ?>
					</div>
				</div>
			</div>
		</div>
