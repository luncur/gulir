...
if ( ! function_exists( 'gulir_get_grid_personalize_2' ) ) {
	 ...
	 * @return false|string
	 */
	function gulir_get_grid_personalize_2( $settings = [], $_query = null ) {
		...
		if ( gulir_is_edit_mode() ) {
			gulir_live_block_grid_personalize_2( $settings );
		} else {
			if ( empty( $settings['display_mode'] ) ) {
...
if ( ! function_exists( 'gulir_loop_grid_personalize_2' ) ) {
	/**
	 * @param  $settings
	 * @param  $_query
	 */
	function gulir_loop_grid_personalize_2( $settings, $_query ) {
		...
		while ( $_query->have_posts() ) :
			...
			gulir_grid_flex_2( $settings );
		endwhile;
	}
}

if ( ! function_exists( 'gulir_live_block_grid_personalize_2' ) ) {
	/**
	 * @param array $settings
	 *
	 * @return false|string
	 */
	function gulir_live_block_grid_personalize_2( $settings = [] ) {
		...
		if ( empty( $_query ) || ! $_query->have_posts() ) {
			...
			gulir_loop_grid_personalize_2( $settings, $_query );
