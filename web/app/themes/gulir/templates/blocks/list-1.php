...
if ( ! function_exists( 'gulir_get_list_1' ) ) {
	 ...
	 * @return false|string
	 */
	function gulir_get_list_1( $settings = [], $_query = null ) {
		...
		$settings['classes'] = 'block-big block-list block-list-1';
		$min_posts           = 1;
		...
		$settings = gulir_get_design_standard_block( $settings, 'list_1' );
		...
		if ( ! $_query->have_posts() || $_query->post_count < $min_posts ) {
			...
			gulir_loop_list_1( $settings, $_query );
...
if ( ! function_exists( 'gulir_loop_list_1' ) ) {
	/**
	 * @param  $settings
	 * @param  $_query
	 */
	function gulir_loop_list_1( $settings, $_query ) {
