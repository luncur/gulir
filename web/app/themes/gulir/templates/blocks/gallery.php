...
if ( ! function_exists( 'gulir_get_simple_gallery' ) ) {
	 ...
	 * @return false|string
	 */
	function gulir_get_simple_gallery( $settings = [], $_query = null ) {
		...
		if ( is_array( $settings['gallery_data'] ) ) {
			foreach ( $settings['gallery_data'] as $item ) {
				...
				echo gulir_get_simple_gallery_item( $item, $is_lazy );
			}
		}
...
if ( ! function_exists( 'gulir_get_simple_gallery_item' ) ) {
	function gulir_get_simple_gallery_item( $item = [], $is_lazy = true ) {
		...
		if ( ! empty( $item['image']['id'] ) ) {
			...
			$image = '<img src="' . $image['url'] . '" alt="' . esc_html__( 'gallery image', 'gulir' ) . '" ' . $attr . '>';
		}
		if ( ! empty( $image ) ) {
			$output .= '<div class="simple-gallery-image">';
			if ( ! empty( $item['link']['url'] ) ) {
				$output .= gulir_render_elementor_link( $item['link'], $image, '', $item['title'] );
			} else {
				$output .= $image;
			}
			if ( ! empty( $item['meta'] ) ) {
		...
		if ( ! empty( $item['title'] ) ) {
			...
			if ( ! empty( $item['link']['url'] ) ) {
				$output .= gulir_render_elementor_link( $item['link'], esc_html( $item['title'] ) );
			} else {
				$output .= gulir_strip_tags( $item['title'] );
			}
			$output .= '</span>';
		}
		...
		if ( ! empty( $item['description'] ) ) {
			...
			$output .= gulir_strip_tags( $item['description'] );
			$output .= '</span>';
		}
...
if ( ! function_exists( 'gulir_get_lightbox_gallery' ) ) {
	function gulir_get_lightbox_gallery( $settings = [], $_query = null ) {
		...
		$output = gulir_get_block_open_tag( $settings, null );
		...
		if ( is_array( $settings['gallery_data'] ) ) {
			foreach ( $settings['gallery_data'] as $item ) {
				...
				$data['title']   = ! empty( $item['title'] ) ? gulir_strip_tags( $item['title'] ) : get_the_title( $attachment_id );
				$data['excerpt'] = ! empty( $item['excerpt'] ) ? gulir_strip_tags( $item['excerpt'] ) : wp_get_attachment_caption( $attachment_id );

				if ( empty( $item['description'] ) ) {
					$attachment          = get_post( $attachment_id );
					$data['description'] = gulir_strip_tags( $attachment->post_content );
				} else {
					$data['description'] = gulir_strip_tags( $item['description'] );
				}
				...
				$output .= '<div class="p-wrap gallery-popup-trigger lightbox-gallery-item" data-index="' . esc_attr( $key ) . '">';
