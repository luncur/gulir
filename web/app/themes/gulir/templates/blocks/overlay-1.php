...
if ( ! function_exists( 'gulir_get_overlay_1' ) ) {
	 ...
	 * @return false|string
	 */
	function gulir_get_overlay_1( $settings = [], $_query = null ) {
		...
		if ( ! $_query->have_posts() || $_query->post_count < $min_posts ) {
			...
			gulir_loop_overlay_1( $settings, $_query );
...
if ( ! function_exists( 'gulir_loop_overlay_1' ) ) {
	/**
	 * @param $settings
	 * @param $_query
	 */
	function gulir_loop_overlay_1( $settings, $_query ) {

		$loop_index = 1;
		if ( ! empty( $settings['slider'] ) && $_query->post_count > 1 ) : ?>
			<div class="post-slider swiper-container pre-load" <?php gulir_slider_attrs( $settings ); ?>>
