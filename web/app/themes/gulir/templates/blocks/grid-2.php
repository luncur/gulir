...
if ( ! function_exists( 'gulir_get_grid_2' ) ) {
	 ...
	 * @return false|string
	 */
	function gulir_get_grid_2( $settings = [], $_query = null ) {
		...
		$settings['classes'] = 'block-grid block-grid-2';
		...
		$settings = gulir_get_design_standard_block( $settings, 'grid_2' );
		...
		if ( ! $_query->have_posts() ) {
			...
			gulir_loop_grid_2( $settings, $_query );
...
if ( ! function_exists( 'gulir_loop_grid_2' ) ) {
	/**
	 * @param  $settings
	 * @param  $_query
	 */
	function gulir_loop_grid_2( $settings, $_query ) {
