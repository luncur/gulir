...
if ( ! function_exists( 'gulir_get_grid_1' ) ) {
	 ...
	 * @return false|string
	 */
	function gulir_get_grid_1( $settings = [], $_query = null ) {
		...
		$settings = gulir_get_design_standard_block( $settings, 'grid_1' );
		...
		if ( ! $_query->have_posts() ) {
			...
			gulir_loop_grid_1( $settings, $_query );
...
if ( ! function_exists( 'gulir_loop_grid_1' ) ) {
	/**
	 * @param  $settings
	 * @param  $_query
	 */
	function gulir_loop_grid_1( $settings, $_query ) {
