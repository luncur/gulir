...
if ( ! function_exists( 'gulir_get_newsletter' ) ) {
	function gulir_get_newsletter( $settings = [] ) {
		...
		$output .= gulir_get_newsletter_background( $settings );

		$output .= '<div class="newsletter-inner">';
		$output .= gulir_get_newsletter_featured( $settings );
		if ( ! empty( $settings['title'] ) || ! empty( $settings['description'] ) ) {
			$output .= '<div class="newsletter-content">';
			if ( ! empty( $settings['title'] ) ) {
				$output .= '<' . $settings['title_tag'] . ' class="newsletter-title">' . gulir_strip_tags( $settings['title'] );
				$output .= '</' . $settings['title_tag'] . '>';
			}
			if ( ! empty( $settings['description'] ) ) {
				$output .= '<div class="newsletter-description rb-text">' . gulir_strip_tags( $settings['description'] ) . '</div>';
			}
			$output .= '</div>';
		}
...
if ( ! function_exists( 'gulir_get_sidebar_newsletter' ) ) {
	function gulir_get_sidebar_newsletter( $settings = [] ) {
...
if ( ! function_exists( 'gulir_get_newsletter_background' ) ) {
	function gulir_get_newsletter_background( $settings = [] ) {
		...
		$output .= gulir_e_image_tag( $default, $dark, $is_lazy );
		$output .= '</div>';
...
if ( ! function_exists( 'gulir_get_newsletter_featured' ) ) {
	function gulir_get_newsletter_featured( $settings = [] ) {
...
if ( ! function_exists( 'gulir_e_image_tag' ) ) {
	function gulir_e_image_tag( $default = [], $dark = [], $is_lazy = true ) {
