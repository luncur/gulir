...
if ( ! function_exists( 'gulir_get_list_box_1' ) ) {
	 ...
	 * @return false|string
	 */
	function gulir_get_list_box_1( $settings = [], $_query = null ) {
		...
		$settings = gulir_get_design_standard_block( $settings, 'list_box_1' );
		...
		if ( ! $_query->have_posts() || $_query->post_count < $min_posts ) {
			...
			gulir_loop_list_box_1( $settings, $_query );
...
if ( ! function_exists( 'gulir_loop_list_box_1' ) ) {
	/**
	 * @param  $settings
	 * @param  $_query
	 */
	function gulir_loop_list_box_1( $settings, $_query ) {
