...
if ( ! function_exists( 'gulir_get_grid_personalize_1' ) ) {
	 ...
	 * @return false|string
	 */
	function gulir_get_grid_personalize_1( $settings = [], $_query = null ) {
		...
		if ( gulir_is_edit_mode() ) {
			gulir_live_block_grid_personalize_1( $settings );
		} else {
			if ( empty( $settings['display_mode'] ) ) {
...
if ( ! function_exists( 'gulir_live_block_grid_personalize_1' ) ) {
	/**
	 * @param array $settings
	 *
	 * @return false|string
	 */
	function gulir_live_block_grid_personalize_1( $settings = [] ) {
		...
		if ( empty( $_query ) || ! $_query->have_posts() ) {
			...
			gulir_loop_grid_personalize_1( $settings, $_query );
...
if ( ! function_exists( 'gulir_loop_grid_personalize_1' ) ) {
	/**
	 * @param  $settings
	 * @param  $_query
	 */
	function gulir_loop_grid_personalize_1( $settings, $_query ) {
