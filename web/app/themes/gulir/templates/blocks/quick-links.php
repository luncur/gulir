...
if ( ! function_exists( 'gulir_get_quick_links' ) ) {
	function gulir_get_quick_links( $settings = [] ) {
		...
		switch ( $settings['source'] ) {
			case 'tax' :
				$inner = gulir_quick_items_from_tax( $settings );
				break;
			case 'both' :
				$inner = gulir_quick_items_from_input( $settings );
				$inner .= gulir_quick_items_from_tax( $settings );
				break;
			case 'sub' :
				$inner = gulir_quick_items_from_subterms( $settings );
				$inner .= gulir_quick_items_from_input( $settings );
				break;
			default :
				$inner = gulir_quick_items_from_input( $settings );
		...
		$output = gulir_get_block_open_tag( $settings );

		$output .= '<ul class="qlinks-inner">';
		if ( ! empty( $settings['header'] ) ) {
			$output .= '<li class="qlink qlinks-heading">';
			$output .= '<div class="qlink-label">';
			$output .= gulir_strip_tags( $settings['header'] );
			$output .= '</div></li>';
		}
		$output .= $inner;
		$output .= '</ul>';
		$output .= gulir_get_block_close_tag();

		return $output;
	}
}

if ( ! function_exists( 'gulir_quick_items_from_input' ) ) {
	function gulir_quick_items_from_input( $settings ) {
		...
		foreach ( $settings['quick_links'] as $item ) {
			...
			$output .= '<li class="qlink h5">' . gulir_render_elementor_link( $item['url'], $title ) . '</li>';
		}
...
if ( ! function_exists( 'gulir_quick_items_from_tax' ) ) {
	function gulir_quick_items_from_tax( $settings ) {
...
if ( ! function_exists( 'gulir_quick_items_from_subterms' ) ) {
	function gulir_quick_items_from_subterms( $settings = [] ) {
		...
		foreach ( $subterms as $term ) {
			$output .= '<li class="qlink h5"><a href="' . esc_url( get_term_link( $term ) ) . '">' . gulir_strip_tags( $term->name ) . '</a></li>';
		}
