...
if ( ! function_exists( 'gulir_get_list_small_2' ) ) {
	 ...
	 * @return false|string
	 */
	function gulir_get_list_small_2( $settings = [], $_query = null ) {
		...
		$settings['classes'] = 'block-small block-list block-list-small-2';
		...
		if ( ! $_query->have_posts() || $_query->post_count < $min_posts ) {
			...
			gulir_loop_list_small_2( $settings, $_query );
...
if ( ! function_exists( 'gulir_loop_list_small_2' ) ) {
	/**
	 * @param  $settings
	 * @param  $_query
	 */
	function gulir_loop_list_small_2( $settings, $_query ) {
