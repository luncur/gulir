...
if ( ! function_exists( 'gulir_get_grid_box_1' ) ) {
	 ...
	 * @return false|string
	 */
	function gulir_get_grid_box_1( $settings = [], $_query = null ) {
		...
		$settings['classes'] = 'block-grid block-grid-box-1';

		if ( ! $_query ) {
			$_query = gulir_query( $settings );
		}

		$settings = gulir_get_design_standard_block( $settings, 'grid_box_1' );
		...
		if ( ! $_query->have_posts() ) {
			...
			gulir_loop_grid_box_1( $settings, $_query );
...
if ( ! function_exists( 'gulir_loop_grid_box_1' ) ) {
	/**
	 * @param  $settings
	 * @param  $_query
	 */
	function gulir_loop_grid_box_1( $settings, $_query ) {
