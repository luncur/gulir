...
if ( ! function_exists( 'gulir_get_list_small_1' ) ) {
	 ...
	 * @return false|string
	 */
	function gulir_get_list_small_1( $settings = [], $_query = null ) {
		...
		if ( ! $_query->have_posts() || $_query->post_count < $min_posts ) {
			...
			gulir_loop_list_small_1( $settings, $_query );
			gulir_block_inner_close_tag( $settings );
			gulir_render_pagination( $settings, $_query );
			wp_reset_postdata();
		}
		gulir_block_close_tag( $settings );

		return ob_get_clean();
	}
}

if ( ! function_exists( 'gulir_loop_list_small_1' ) ) {
	/**
	 * @param  $settings
	 * @param  $_query
	 */
	function gulir_loop_list_small_1( $settings, $_query ) {
