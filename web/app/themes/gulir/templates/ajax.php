...
if ( ! class_exists( 'Gulir_Ajax', false ) ) {
	class Gulir_Ajax {
		...
		public function pagination() {
			...
			if ( ! empty( $settings['content_source'] ) ) {
				switch ( $settings['content_source'] ) {
					case 'related':
						$_query = gulir_query_related( $settings, $paged );
						break;
					case 'recommended':
					...
					case 'saved':
						$settings['paged'] = $paged;
						$_query            = Gulir_Personalize::get_instance()->saved_posts_query( $settings );
						break;
					case 'history':
						$settings['paged'] = $paged;
						$_query            = Gulir_Personalize::get_instance()->reading_history_query( $settings );
						break;
				}
			} else {
				$_query = gulir_query( $settings, $paged );
			}
		...
		function validate( $settings ) {
			...
			if ( is_array( $settings ) ) {
				...
				return gulir_strip_tags( stripslashes( $settings ) );
			}
		...
		function render( $settings, $_query ) {
			...
			ob_start();
			$func = 'gulir_loop_' . trim( $settings['name'] );
		...
		function end_list_info() {
			...
			$output .= gulir_html__( 'You\'ve reached the end of the list!', 'gulir' );
			$output .= '</span></div>';
		...
		public function search_posts() {
			...
			$limit     = ! empty( $_GET['limit'] ) ? absint( $_GET['limit'] ) : gulir_get_option( 'ajax_search_limit', 4 );
			...
			if ( $_query->have_posts() ) {
				...
				$response .= '<div class="live-search-link"><a class="is-btn" href="' . get_search_link( $input ) . '">' . gulir_html__( 'More Results', 'gulir' ) . '</a></div>';
			} else {
				$response .= '<div class="search-no-result">' . gulir_html__( 'Sorry, but nothing matched your search terms. Please try again with some different keywords.', 'gulir' ) . '</div>';
			}
		...
		public function personalize_block() {
			...
			$func = 'gulir_live_block_' . trim( $settings['name'] );
		...
		public function personalize_categories() {
			...
			$settings = $this->validate( $_GET['data'] );
			$func     = 'gulir_live_get_' . trim( $settings['name'] );
		...
		function notification() {
			...
			$duration = absint( gulir_get_option( 'notification_duration' ) );
			if ( empty( $duration ) ) {
			...
			if ( empty( $response['content'] ) ) {
				$response['content'] = '<span class="is-meta empty-notification">' . gulir_html__( 'Stay Tuned! Check back later for the latest updates.', 'gulir' ) . '</span>';
			}
		...
		public function voting() {
			...
			if ( empty( $_GET['pid'] ) || ! class_exists( 'Gulir_Personalize_Helper' ) || empty( $_GET['value'] ) ) {
				wp_send_json_error();
			}
			...
			switch ( $reaction ) {
				case 'like':
					Gulir_Personalize_Helper::get_instance()->save_vote( 'like', $post_id );
					break;
				case 'dislike':
					Gulir_Personalize_Helper::get_instance()->save_vote( 'dislike', $post_id );
					break;
				case 'rmlike' :
					Gulir_Personalize_Helper::get_instance()->delete_vote( 'like', $post_id );
					break;
				case 'rmdislike' :
					Gulir_Personalize_Helper::get_instance()->delete_vote( 'dislike', $post_id );
					break;
			}
