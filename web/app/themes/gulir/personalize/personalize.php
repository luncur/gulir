...
if ( ! class_exists( 'Gulir_Personalize', false ) ) {
	class Gulir_Personalize {
		...
		public function __construct() {
			...
			if ( self::$enabled ) {
				...
				add_action( 'wp_footer', 'gulir_bookmark_info_template' );
		...
		public function get_settings() {
			...
			self::$notification = gulir_get_option( 'bookmark_notification' );
			self::$enable_when  = gulir_get_option( 'bookmark_enable_when' );
			self::$enabled      = gulir_get_option( 'bookmark_system' );
		}
		...
		public function bookmark_toggle() {
			...
			if ( empty( $_POST['pid'] ) || ! class_exists( 'Gulir_Personalize_Helper' ) || ! gulir_get_option( 'bookmark_system' ) ) {
				wp_send_json( [] );
			}
			...
			if ( empty( $_POST['type'] ) || 'save' == $_POST['type'] ) {
				...
				$response['description'] = gulir_html__( 'This article has been added to reading list', 'gulir' );
				Gulir_Personalize_Helper::get_instance()->save_bookmark( $post_id );
			} else {
				$response['action']      = 'removed';
				$response['description'] = gulir_html__( 'This article was removed from your bookmark', 'gulir' );
				Gulir_Personalize_Helper::get_instance()->delete_bookmark( $post_id );
			}
		...
		function category_follow_toggle() {
			...
			if ( empty( $_POST['cid'] ) || ! class_exists( 'Gulir_Personalize_Helper' ) || ! gulir_get_option( 'bookmark_system' ) ) {
				wp_send_json( [] );
			}
			...
			$response    = [
				'action'      => 'saved',
				'description' => gulir_html__( 'You are now following', 'gulir' ),
			];

			if ( empty( $_POST['type'] ) || 'follow' == $_POST['type'] ) {
				Gulir_Personalize_Helper::get_instance()->save_category( $category_id );
			} else {
				$response['action']      = 'removed';
				$response['description'] = gulir_html__( 'You are no longer following', 'gulir' );
				Gulir_Personalize_Helper::get_instance()->delete_category( $category_id );
			}
		...
		function writer_follow_toggle() {
			...
			if ( empty( $_POST['uid'] ) || ! class_exists( 'Gulir_Personalize_Helper' ) || ! gulir_get_option( 'bookmark_system' ) ) {
				wp_send_json( [] );
			}
			...
			if ( empty( $_POST['type'] ) || 'follow' == $_POST['type'] ) {
				Gulir_Personalize_Helper::get_instance()->save_writer( $writer_id );
			} else {
				$response['action']      = 'removed';
				$response['description'] = gulir_html__( 'You are no longer following', 'gulir' );
				Gulir_Personalize_Helper::get_instance()->delete_writer( $writer_id );
			}
		...
		public function sync_data() {
			...
			if ( ! gulir_get_option( 'bookmark_system' ) || ! is_user_logged_in() || ! class_exists( 'Gulir_Personalize_Helper' ) ) {
				wp_send_json( [] );
				wp_die();
			}

			$data = [
				'bookmarks'  => Gulir_Personalize_Helper::get_instance()->get_bookmarks(),
				'categories' => Gulir_Personalize_Helper::get_instance()->get_categories_followed(),
				'writers'    => Gulir_Personalize_Helper::get_instance()->get_writers_followed(),
			];
		...
		public function get_categories_followed() {
			...
			if ( ! gulir_get_option( 'bookmark_system' ) || ! class_exists( 'Gulir_Personalize_Helper' ) ) {
				return [];
			}
			...
			return Gulir_Personalize_Helper::get_instance()->get_categories_followed();
		}
		...
		public function get_writers_followed() {
			...
			return Gulir_Personalize_Helper::get_instance()->get_writers_followed();
		}
		...
		public function get_my_writers() {
			...
			$role = [ 'author', 'editor' ];
			if ( gulir_get_option( 'bookmark_author_admin' ) ) {
				$role[] = 'administrator';
			}
		...
		public function saved_posts_query( $settings = [] ) {
			...
			$data = Gulir_Personalize_Helper::get_instance()->get_bookmarks();
		...
		public function recommended_pre_query( $settings = [] ) {
			...
			$settings['posts_per_page'] = empty( $settings['posts_per_page'] ) ? gulir_get_option( 'recommended_posts_per_page', get_option( 'posts_per_page' ) ) : $settings['posts_per_page'];
		...
		function reading_list() {
			...
			$response = [
				'saved'       => gulir_my_saved_listing(),
				'categories'  => gulir_my_categories_listing(),
				'writers'     => gulir_my_writers_listing(),
				'recommended' => gulir_my_recommended_listing(),
			];
			wp_send_json( $response );
		}
		...
		public function add_history() {
			...
			if ( empty( $_GET['id'] ) || ! class_exists( 'Gulir_Personalize_Helper' ) ) {
				wp_send_json_error();
			}

			$post_id = absint( $_GET['id'] );
			Gulir_Personalize_Helper::get_instance()->save_history( $post_id );
		...
		public function reading_history_query( $settings = [] ) {
			...
			$data = Gulir_Personalize_Helper::get_instance()->get_history();
...
/** load */
Gulir_Personalize::get_instance();