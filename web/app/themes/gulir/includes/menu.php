...
add_filter( 'nav_menu_item_title', 'gulir_add_menu_title', 10, 4 );

if ( ! function_exists( 'gulir_add_menu_title' ) ) {
	function gulir_add_menu_title( $title, $item, $args, $depth ) {

		$settings = rb_get_cached_nav_item_meta( 'gulir_menu_meta', $item->ID );

		$output = '<span>';
		if ( ! empty( $settings['icon'] ) ) {
			$output .= '<i class="menu-item-icon ' . esc_attr( $settings['icon'] ) . '" aria-hidden="true"></i>';
		} elseif ( ! empty( $settings['icon_id'] ) ) {
			$output .= '<span class="menu-item-svg">' . gulir_get_svg_content( $settings['icon_id'] ) . '</span>';
...
if ( ! class_exists( 'Gulir_Walker_Nav_Menu', false ) ) {
	class Gulir_Walker_Nav_Menu extends Walker_Nav_Menu {
		...
		public function blog_default( $item ) {
			...
			if ( ! empty( $item->object_id ) ) {
				...
				$output .= '><span>' . gulir_html__( 'Show More', 'gulir' ) . '</span><i class="rbi rbi-cright" aria-hidden="true"></i></a>';
				$output .= '</div>';

				$output .= gulir_get_grid_small_1( [
						'uuid'           => 'mega-listing-' . $item->ID,
						'columns'        => 5,
						'posts_per_page' => 5,
						'title_tag'      => 'div',
						'crop_size'      => 'gulir_crop_g1',
						'title_classes'  => 'h4',
						'review'         => 'replace',
						'entry_meta'     => 'update',
						'entry_category' => '-1',
		...
		public function blog_hierarchical( $item ) {
			...
			if ( ! empty( $item->object_id ) ) {
				$data        = rb_get_term_meta( 'gulir_category_meta', $item->object_id );
				...
				$output .= '<div class="category-hero-wrap">' . gulir_get_category_hero( $featured ) . '</div>';
				...
				$output .= '<i class="rbi rbi-trending" aria-hidden="true"></i><span class="h4">' . gulir_html__( 'Top News', 'gulir' ) . '</span>';
				$output .= '</div>';

				$output .= gulir_get_list_small_2( [
				...
				$output .= '<div class="mega-header">';
				$output .= '<i class="rbi rbi-clock" aria-hidden="true"></i><span class="h4">' . gulir_html__( 'Latest News', 'gulir' ) . '</span>';
				$output .= '</div>';

				$output .= gulir_get_list_small_1( [
