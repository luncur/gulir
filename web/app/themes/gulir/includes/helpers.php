...
if ( ! function_exists( 'gulir_get_user_socials' ) ) {
	function gulir_get_user_socials( $author_id = '' ) {
...
if ( ! function_exists( 'gulir_is_wc_pages' ) ) {
	function gulir_is_wc_pages() {
...
if ( ! function_exists( 'gulir_get_video_mine_type' ) ) {
	/**
	 * @param $url
	 *
	 * @return string
	 */
	function gulir_get_video_mine_type( $url ) {
...
if ( ! function_exists( 'gulir_get_post_likes' ) ) {
	/**
	 * @param string $id
	 *
	 * @return false|string
	 */
	function gulir_get_post_likes( $id = '' ) {

		$count = get_post_meta( $id, 'rb_total_like', true );
		if ( ! empty( $count ) ) {
			return gulir_pretty_number( $count );
		}
...
if ( ! function_exists( 'gulir_get_post_dislikes' ) ) {
	/**
	 * @param string $id
	 *
	 * @return false|string
	 */
	function gulir_get_post_dislikes( $id = '' ) {
 ...
 * @return array
 */
if ( ! function_exists( 'gulir_get_top_post_ids' ) ) {
	function gulir_get_top_post_ids( $settings = [] ) {
		...
		$data = gulir_convert_stats_array_to_object( ( new Automattic\Jetpack\Stats\WPCOM_Stats() )->get_top_posts( $args ) );
 ...
 */
if ( ! function_exists( 'gulir_convert_stats_array_to_object' ) ) {
	function gulir_convert_stats_array_to_object( $stats_array ) {
...
if ( ! function_exists( 'gulir_get_dynamic_css' ) ) {
	/**
	 * @return string
	 */
	function gulir_get_dynamic_css() {

		$output = stripslashes( get_option( 'gulir_style_cache', '' ) );
		$output .= stripslashes( get_option( 'gulir_term_style_cache', '' ) );

		/** direct css to fix encoding issues */
		$exclusive = gulir_get_option( 'exclusive_label' );
		$live      = gulir_get_option( 'live_label', 'live:' ) . ' ';

		if ( ! empty( $exclusive ) ) {
			$output .= '.entry-title.is-p-protected a:before { content: "' . esc_attr( $exclusive ) . '";display: inline-block; }';
		}
		if ( 'dot' !== gulir_get_option( 'live_blog_meta' ) ) {
...
if ( ! function_exists( 'gulir_get_content_images' ) ) {
	/**
	 * @param string $post_id
	 *
	 * @return array|mixed|string
	 */
	function gulir_get_content_images( $post_id = '' ) {
...
if ( ! function_exists( 'gulir_is_review_post' ) ) {
	/**
	 * @param string $post_id
	 *
	 * @return bool
	 */
	function gulir_is_review_post( $post_id = '' ) {
...
if ( ! function_exists( 'gulir_is_sponsored_post' ) ) {
	function gulir_is_sponsored_post( $post_id = '' ) {
...
if ( ! function_exists( 'gulir_is_live_blog' ) ) {
	function gulir_is_live_blog( $post_id = '' ) {
...
if ( ! function_exists( 'gulir_get_header_style' ) ) {
	/**
	 * @return false|mixed|string|void
	 */
	function gulir_get_header_style() {
		...
		if ( is_singular( 'podcast' ) ) {
			...
			$default_template = trim( gulir_get_option( 'single_podcast_header_template' ) );
			...
			if ( gulir_get_option( 'single_podcast_header_style' ) ) {
				$params['style'] = gulir_get_option( 'single_podcast_header_style' );
			}
		} elseif ( is_page() ) {
			...
			if ( $post_format ) {
				...
				$format_template = trim( gulir_get_option( $option_name ) );
			...
			$default_template = trim( gulir_get_option( 'single_post_header_template' ) );
			if ( ! empty( $default_template ) ) {
				return [
						'style'     => 'rb_template',
						'shortcode' => $default_template,
			...
			if ( gulir_get_option( 'single_post_header_style' ) ) {
				$params['style'] = gulir_get_option( 'single_post_header_style' );
			}
		} elseif ( is_category() ) {

			$categories_data = rb_get_term_meta( 'gulir_category_meta', get_queried_object_id() );
			...
			if ( empty( $categories_data['header_style'] ) ) {
				...
				$default_template = trim( gulir_get_option( 'category_header_template' ) );
				if ( ! empty( $default_template ) ) {
					return [
							'style'     => 'rb_template',
							'shortcode' => $default_template,
				...
				if ( gulir_get_option( 'category_header_style' ) ) {
					$params['style'] = gulir_get_option( 'category_header_style' );
				}
			} else {
			...
			$default_template = trim( gulir_get_option( 'search_header_template' ) );
			if ( $default_template ) {
				return [
						'style'     => 'rb_template',
						'shortcode' => $default_template,
			...
			if ( gulir_get_option( 'search_header_style' ) ) {
				$params['style'] = gulir_get_option( 'search_header_style' );
			}
		} elseif ( is_home() ) {

			$default_template = trim( gulir_get_option( 'blog_header_template' ) );
			if ( $default_template ) {
				return [
						'style'     => 'rb_template',
						'shortcode' => $default_template,
			...
			if ( gulir_get_option( 'blog_header_style' ) ) {
				$params['style'] = gulir_get_option( 'blog_header_style' );
			}
			...
			if ( empty( $categories_data['header_style'] ) ) {
				...
				$default_template = trim( gulir_get_option( 'series_header_template' ) );
				if ( ! empty( $default_template ) ) {
					return [
							'style'     => 'rb_template',
							'shortcode' => $default_template,
				...
				if ( gulir_get_option( 'series_header_style' ) ) {
					$params['style'] = gulir_get_option( 'series_header_style' );
				}
			} else {
				$params['style'] = $categories_data['header_style'];
			}
			...
			$tag_data = rb_get_term_meta( 'gulir_category_meta', get_queried_object_id() );
			...
			$tax_data = rb_get_term_meta( 'gulir_category_meta', get_queried_object_id() );
		...
		if ( empty( $params['style'] ) || 'default' === $params['style'] ) {
			...
			$params['style'] = gulir_get_option( 'header_style' );
			if ( 'rb_template' === $params['style'] ) {
				if ( gulir_get_option( 'header_template' ) ) {
					$params['shortcode'] = gulir_get_option( 'header_template' );
				} else {
					$params['style'] = 1;
				}
			}
...
if ( ! function_exists( 'gulir_get_header_settings' ) ) {
	/**
	 * @param $prefix
	 *
	 * @return array
	 * get header settings
	 */
	function gulir_get_header_settings( $prefix ) {

		$prefix = trim( $prefix ) . '_';

		$settings = gulir_get_option();

		$settings['more']       = gulir_get_option( $prefix . 'more' );
		$settings['nav_style']  = gulir_get_option( $prefix . 'nav_style' );
		$settings['sub_scheme'] = gulir_get_option( 'hd1_sub_scheme' );
		...
		$settings['header_socials'] = gulir_get_option( $prefix . 'header_socials' );
		if ( empty( $settings['transparent_mobile_logo']['url'] ) ) {
			$settings['transparent_mobile_logo'] = gulir_get_option( 'transparent_logo' );
		}
...
if ( ! function_exists( 'gulir_get_design_standard_block' ) ) {
	/**
	 * @param array $settings
	 * @param string $prefix
	 *
	 ...
	 * get module settings
	 */
	function gulir_get_design_standard_block( $settings = [], $prefix = '' ) {
		...
		if ( empty( $settings['crop_size'] ) ) {
			$settings['crop_size'] = gulir_get_option( $prefix . 'crop_size' );
		}

		if ( empty( $settings['featured_position'] ) ) {
			$settings['featured_position'] = gulir_get_option( $prefix . 'featured_position' );
		}

		if ( empty( $settings['entry_category'] ) ) {
			$settings['entry_category'] = gulir_get_option( $prefix . 'entry_category' );
		} elseif ( '-1' === (string) $settings['entry_category'] ) {
			$settings['entry_category'] = false;
		}

		if ( empty( $settings['hide_category'] ) ) {
			$settings['hide_category'] = gulir_get_option( $prefix . 'hide_category' );
		...
		if ( ! empty( $settings['entry_meta_bar'] ) && '-1' === (string) $settings['entry_meta_bar'] ) {
			...
			if ( empty( $settings['entry_meta_bar'] ) || 'custom' !== (string) $settings['entry_meta_bar'] ) {
				$settings['entry_meta'] = gulir_get_option( $prefix . 'entry_meta' );
			} else {
		...
		if ( empty( $settings['tablet_hide_meta'] ) ) {
			$settings['tablet_hide_meta'] = gulir_get_option( $prefix . 'tablet_hide_meta' );
		} else {
		...
		if ( empty( $settings['mobile_hide_meta'] ) ) {
			$settings['mobile_hide_meta'] = gulir_get_option( $prefix . 'mobile_hide_meta' );
		} else {
		...
		if ( empty( $settings['review'] ) ) {
			$settings['review'] = gulir_get_option( $prefix . 'review' );
		} elseif ( '-1' === (string) $settings['review'] ) {
			$settings['review'] = false;
		}

		if ( empty( $settings['review_meta'] ) ) {
			$settings['review_meta'] = gulir_get_option( $prefix . 'review_meta' );
		} elseif ( '-1' === (string) $settings['review_meta'] ) {
			$settings['review_meta'] = false;
		}

		if ( empty( $settings['entry_format'] ) ) {
			$settings['entry_format'] = gulir_get_option( $prefix . 'entry_format' );
		} elseif ( '-1' === (string) $settings['entry_format'] ) {
			$settings['entry_format'] = false;
		}

		if ( empty( $settings['bookmark'] ) ) {
			$settings['bookmark'] = gulir_get_option( $prefix . 'bookmark' );
		} elseif ( '-1' === (string) $settings['bookmark'] ) {
			$settings['bookmark'] = false;
		}

		if ( empty( $settings['excerpt'] ) ) {
			$settings['excerpt_length'] = gulir_get_option( $prefix . 'excerpt_length' );
			$settings['excerpt_source'] = gulir_get_option( $prefix . 'excerpt_source' );
		}

		if ( empty( $settings['hide_excerpt'] ) ) {
			$settings['hide_excerpt'] = gulir_get_option( $prefix . 'hide_excerpt' );
		} elseif ( '-1' === (string) $settings['hide_excerpt'] ) {
			$settings['hide_excerpt'] = false;
		}

		if ( empty( $settings['readmore'] ) ) {
			$settings['readmore'] = gulir_get_option( $prefix . 'readmore' );
		} elseif ( '-1' === (string) $settings['readmore'] ) {
			$settings['readmore'] = false;
		}

		if ( ! empty( $settings['readmore'] ) ) {
			$settings['readmore'] = gulir_get_readmore_label();
		}

		if ( empty( $settings['title_tag'] ) ) {
			$settings['title_tag'] = gulir_get_option( $prefix . 'title_tag' );
		}

		if ( empty( $settings['sub_title_tag'] ) ) {
			$settings['sub_title_tag'] = gulir_get_option( $prefix . 'sub_title_tag' );
		}

		if ( empty( $settings['sub_sub_title_tag'] ) ) {
			$settings['sub_sub_title_tag'] = gulir_get_option( $prefix . 'sub_sub_title_tag' );
		}
		...
		if ( ! empty( $settings['sponsor_meta'] ) && '-1' === (string) $settings['sponsor_meta'] ) {
			...
			$settings['sponsor_meta'] = gulir_get_option( $prefix . 'sponsor_meta' );
		}

		if ( empty( $settings['title_classes'] ) ) {
			$settings['title_classes'] = gulir_get_option( $prefix . 'title_classes' );
		}
		if ( empty( $settings['box_style'] ) ) {
			$settings['box_style'] = gulir_get_option( $prefix . 'box_style' );
		}
		if ( empty( $settings['center_mode'] ) ) {
			$settings['center_mode'] = gulir_get_option( $prefix . 'center_mode' );
		} elseif ( '-1' === (string) $settings['center_mode'] ) {
			$settings['center_mode'] = false;
		}
		if ( empty( $settings['middle_mode'] ) ) {
			$settings['middle_mode'] = gulir_get_option( $prefix . 'middle_mode' );
		}
		...
		if ( empty( $settings['slider_fmode'] ) ) {
			$settings['slider_fmode'] = gulir_get_option( 'slider_fmode' );
...
if ( ! function_exists( 'gulir_get_design_builder_block' ) ) {
	/**
	 * @param $settings
	 *
	 * @return array|mixed
	 */
	function gulir_get_design_builder_block( $settings ) {
...
if ( ! function_exists( 'gulir_get_readmore_label' ) ) {
	/**
	 * @return false|mixed|string|void
	 */
	function gulir_get_readmore_label() {

		$label = gulir_get_option( 'readmore_label' );
		if ( empty( $label ) ) {
			return gulir_html__( 'Read More', 'gulir' );
		}
...
if ( ! function_exists( 'gulir_has_featured_image' ) ) {
	/**
	 * @param string $size
	 *
	 * @return bool
	 ...
	 */
	function gulir_has_featured_image( $size = 'full' ) {
...
if ( ! function_exists( 'gulir_detect_dynamic_query' ) ) {
	/**
	 * @param $settings
	 *
	 * @return mixed
	 * gulir_detect_query
	 */
	function gulir_detect_dynamic_query( $settings ) {
		...
		if ( defined( 'JETPACK__VERSION' ) ) {
			if ( ! empty( $settings['jetpack_top_posts'] ) && '1' === (string) $settings['jetpack_top_posts'] ) {
				$settings['post_in'] = gulir_get_top_post_ids( $settings );
...
if ( ! function_exists( 'gulir_get_single_setting' ) ) {
	/**
	 * @param        $name
	 ...
	 */
	function gulir_get_single_setting( $name, $opt_name = '', $post_id = '' ) {
		...
		if ( empty( $setting ) || 'default' === $setting ) {
			...
			$setting = gulir_get_option( $opt_name );
		}
...
if ( ! function_exists( 'gulir_get_review_settings' ) ) {
	/**
	 * @param string $post_id
	 *
	 * @return array|false
	 */
	function gulir_get_review_settings( $post_id = '' ) {

		if ( empty( $post_id ) ) {
			$post_id = get_the_ID();
		}

		if ( ! gulir_is_review_post( $post_id ) ) {
			return false;
		}
		...
		$settings = [
				...
				'type'        => gulir_get_single_setting( 'review_type' ),
				'criteria'    => rb_get_meta( 'review_criteria', $post_id ),
				'user'        => gulir_get_single_setting( 'user_can_review' ),
				'image'       => gulir_get_single_setting( 'review_image' ),
				...
				'schema'      => gulir_get_single_setting( 'review_schema' ),
				'user_rating' => get_post_meta( $post_id, 'gulir_user_rating', true ),
		];
...
if ( ! function_exists( 'gulir_get_single_sidebar_name' ) ) {
	/**
	 * @param string $post_id
	 *
	 * @return array|false|mixed|void
	 */
	function gulir_get_single_sidebar_name( $post_id = '' ) {
		...
		if ( 'post' !== $post_type ) {
			$setting = gulir_get_option( 'single_' . $post_type . '_sidebar_name' );
			if ( ! empty( $setting ) && 'default' !== $setting ) {
				return $setting;
			}
		}

		return gulir_get_option( 'single_post_sidebar_name', 'gulir_sidebar_default' );
	}
}

if ( ! function_exists( 'gulir_get_single_sidebar_position' ) ) {
	/**
	 * @param string $name
	 * @param string $opt_name
	 * @param string $post_id
	 ...
	 * @return false|mixed|string|void
	 */
	function gulir_get_single_sidebar_position( $name = 'sidebar_position', $opt_name = '', $post_id = '' ) {

		if ( gulir_is_amp() && gulir_get_option( 'amp_disable_single_sidebar' ) ) {
			return 'none';
		}
		...
		if ( empty( $setting ) || 'default' === $setting ) {
			$setting = gulir_get_option( 'global_sidebar_position' );
		}
 ...
 */
if ( ! function_exists( 'gulir_get_single_layout' ) ) {
	function gulir_get_single_layout() {

		// Store data layout as a global value to optimize speed.
		if ( ! empty( $GLOBALS['gulir_single_layout'] ) && is_array( $GLOBALS['gulir_single_layout'] ) ) {
			return $GLOBALS['gulir_single_layout'];
		}

		$data = gulir_get_raw_single_layout();

		if ( gulir_is_amp() && 'stemplate' === $data['layout'] ) {
			$data['layout'] = 'standard_7';
		}

		$GLOBALS['gulir_single_layout'] = $data;
 ...
 */
if ( ! function_exists( 'gulir_get_raw_single_layout' ) ) {
	function gulir_get_raw_single_layout() {
		...
		if ( 'post' === $post_type ) {
			...
			switch ( $format ) {
				case 'video' :
					...
					if ( ! empty( $individual_layout ) && 'default' !== $individual_layout ) {
						...
						$template = trim( gulir_get_option( 'single_post_video_template' ) );
						if ( ! empty( $template ) ) {
							$data['layout']    = 'stemplate';
							$data['shortcode'] = $template;
						} else {
							$data['layout'] = gulir_get_option( 'single_post_video_layout' );
						}
					}
					break;
				case 'audio' :
					...
					if ( ! empty( $individual_layout ) && 'default' !== $individual_layout ) {
						...
						$template = trim( gulir_get_option( 'single_post_audio_template' ) );
						if ( ! empty( $template ) ) {
							$data['layout']    = 'stemplate';
							$data['shortcode'] = $template;
						} else {
							$data['layout'] = gulir_get_option( 'single_post_audio_layout' );
						}
					}
					break;
				case 'gallery' :
					...
					if ( ! empty( $individual_layout ) && 'default' !== $individual_layout ) {
						...
						$template = trim( gulir_get_option( 'single_post_gallery_template' ) );
						if ( ! empty( $template ) ) {
							$data['layout']    = 'stemplate';
							$data['shortcode'] = $template;
						} else {
							$data['layout'] = gulir_get_option( 'single_post_gallery_layout' );
						}
					}
					break;
				...
				default:
					...
					if ( ! empty( $individual_layout ) && 'default' !== $individual_layout ) {
						...
						$template = trim( gulir_get_option( 'single_post_standard_template' ) );
						if ( ! empty( $template ) ) {
							$data['layout']    = 'stemplate';
							$data['shortcode'] = $template;
						} else {
							$data['layout'] = gulir_get_option( 'single_post_layout' );
						}
					}
			}
		} else {
			...
			if ( ! empty( $individual_layout ) && 'default' !== $individual_layout ) {
				...
				$post_type_template = trim( gulir_get_option( 'post_type_template_' . $post_type ) );
				if ( ! empty( $post_type_template ) ) {
					$data['layout']    = 'stemplate';
					$data['shortcode'] = $post_type_template;
				} else {
					$post_type_layout = gulir_get_option( 'post_type_layout_' . $post_type );
					if ( ! empty( $post_type_layout ) && 'default' !== $post_type_layout ) {
						$data['layout'] = $post_type_layout;
					} else {
						$post_type_template = trim( gulir_get_option( 'post_type_template' ) );
						if ( ! empty( $post_type_template ) ) {
							$data['layout']    = 'stemplate';
							$data['shortcode'] = $post_type_template;
						} else {
							$data['layout'] = gulir_get_option( 'post_type_layout' );
						}
					}
				}
			}
...
if ( ! function_exists( 'gulir_get_related_data' ) ) {
	/**
	 * @param array $settings
	 *
	 * @return WP_Query
	 */
	function gulir_get_related_data( $settings = [] ) {
		...
		return gulir_query_related( $params );
	}
}

if ( ! function_exists( 'gulir_get_single_sticky_setting' ) ) {
	function gulir_get_single_sticky_setting( $prefix ) {

		$setting = gulir_get_option( $prefix . '_sticky_sidebar' );
		if ( empty( $setting ) || 'default' === $setting ) {
			$setting = gulir_get_option( 'sticky_sidebar' );
		}
...
if ( ! function_exists( 'gulir_get_single_sticky_sidebar' ) ) {
	function gulir_get_single_sticky_sidebar( $prefix = 'single_post' ) {

		$setting = (string) gulir_get_single_sticky_setting( $prefix );
...
if ( ! function_exists( 'gulir_get_category_page_settings' ) ) {
	function gulir_get_category_page_settings( $category_id = '' ) {
		...
		if ( empty( $settings['subcategory'] ) ) {
			$settings['subcategory'] = gulir_get_option( $prefix . 'subcategory' );
		}
		if ( '-1' === (string) $settings['subcategory'] ) {
			$settings['subcategory'] = false;
		}
		if ( empty( $settings['follow_category_header'] ) ) {
			$settings['follow_category_header'] = gulir_get_option( 'follow_category_header' );
		}
		if ( empty( $settings['template'] ) ) {
			$settings['template'] = gulir_get_option( $prefix . 'template' );
		}
		if ( empty( $settings['template_display'] ) ) {
			$settings['template_display'] = gulir_get_option( $prefix . 'template_display' );
		}
		...
		if ( empty( $settings['blog_heading'] ) ) {
			$settings['blog_heading'] = gulir_get_option( $prefix . 'blog_heading' );
		}
		if ( empty( $settings['blog_heading_layout'] ) ) {
			$settings['blog_heading_layout'] = gulir_get_option( $prefix . 'blog_heading_layout' );
		}
		if ( empty( $settings['blog_heading_tag'] ) ) {
			$settings['blog_heading_tag'] = gulir_get_option( $prefix . 'blog_heading_tag' );
		}
		if ( empty( $settings['posts_per_page'] ) ) {
			$settings['posts_per_page'] = gulir_get_option( $prefix . 'posts_per_page' );
		}
		if ( empty( $settings['pagination'] ) ) {
			$settings['pagination'] = gulir_get_option( $prefix . 'pagination' );
		}
		if ( empty( $settings['layout'] ) ) {
			$settings['layout'] = gulir_get_option( $prefix . 'layout' );
		}
		if ( empty( $settings['columns'] ) ) {
			$settings['columns'] = gulir_get_option( $prefix . 'columns' );
		}
		if ( empty( $settings['columns_tablet'] ) ) {
			$settings['columns_tablet'] = gulir_get_option( $prefix . 'columns_tablet' );
		}
		if ( empty( $settings['columns_mobile'] ) ) {
			$settings['columns_mobile'] = gulir_get_option( $prefix . 'columns_mobile' );
		}
		if ( empty( $settings['column_gap'] ) ) {
			$settings['column_gap'] = gulir_get_option( $prefix . 'column_gap' );
		}
		if ( empty( $settings['sidebar_position'] ) ) {
			$settings['sidebar_position'] = gulir_get_option( $prefix . 'sidebar_position' );
		}
		if ( empty( $settings['sidebar_name'] ) || 'default' === $settings['sidebar_name'] ) {
			$settings['sidebar_name'] = gulir_get_option( $prefix . 'sidebar_name' );
		}
		if ( empty( $settings['sticky_sidebar'] ) ) {
			$settings['sticky_sidebar'] = gulir_get_option( $prefix . 'sticky_sidebar' );
			if ( empty( $settings['sticky_sidebar'] ) ) {
				$settings['sticky_sidebar'] = gulir_get_option( 'sticky_sidebar' );
			}
		}
		...
		if ( empty( $settings['entry_meta_bar'] ) ) {
			$settings['entry_meta_bar'] = gulir_get_option( $prefix . 'entry_meta_bar' );
		...
		if ( empty( $settings['excerpt'] ) ) {
			$settings['excerpt'] = gulir_get_option( $prefix . 'excerpt' );
			if ( ! empty( $settings['excerpt'] ) ) {
				$settings['excerpt_length'] = gulir_get_option( $prefix . 'excerpt_length' );
				$settings['excerpt_source'] = gulir_get_option( $prefix . 'excerpt_source' );
			}
...
if ( ! function_exists( 'gulir_get_archive_page_settings' ) ) {
	function gulir_get_archive_page_settings( $prefix = '', $settings = [] ) {
		...
		$settings['archive_header']      = gulir_get_option( $prefix . 'archive_header' );
		$settings['blog_heading']        = gulir_get_option( $prefix . 'blog_heading' );
		$settings['blog_heading_layout'] = gulir_get_option( $prefix . 'blog_heading_layout' );
		$settings['blog_heading_tag']    = gulir_get_option( $prefix . 'blog_heading_tag' );
		$settings['pattern']             = gulir_get_option( $prefix . 'pattern' );
		$settings['template']            = gulir_get_option( $prefix . 'template' );
		$settings['template_bottom']     = gulir_get_option( $prefix . 'template_bottom' );
		$settings['template_display']    = gulir_get_option( $prefix . 'template_display' );
		$settings['template_global']     = gulir_get_option( $prefix . 'template_global' );
		$settings['breadcrumb']          = gulir_get_option( $prefix . 'breadcrumb' );
		$settings['posts_per_page']      = gulir_get_option( $prefix . 'posts_per_page' );
...
if ( ! function_exists( 'gulir_get_flex_archive_settings' ) ) {
	function gulir_get_flex_archive_settings() {

		$archive  = get_queried_object();
		$settings = gulir_get_archive_page_settings();

		if ( is_tax() ) {
			$tax_id           = get_queried_object_id();
			$data             = rb_get_term_meta( 'gulir_category_meta', $tax_id );
			$settings['uuid'] = 'uid_tax_' . $tax_id;

			$archive_header  = ! empty( $data['archive_header'] ) ? $data['archive_header'] : gulir_get_option( $archive->taxonomy . '_tax_header', gulir_get_option( 'tax_header' ) );
			$template_global = ! empty( $data['template_global'] ) ? $data['template_global'] : gulir_get_option( $archive->taxonomy . '_tax_template_global', gulir_get_option( 'tax_template_global' ) );
			$posts_per_page  = ! empty( $data['posts_per_page'] ) ? $data['posts_per_page'] : gulir_get_option( $archive->taxonomy . '_tax_posts_per_page', gulir_get_option( 'tax_posts_per_page' ) );
		...
		if ( is_post_type_archive() ) {
			...
			$archive_header  = gulir_get_option( $key . '_archive_header' );
			$template_global = gulir_get_option( $key . '_archive_template_global' );
			$posts_per_page  = gulir_get_option( $key . '_archive_posts_per_page' );
			if ( ! empty( $archive_header ) ) {
				$settings['archive_header'] = $archive_header;
			}
			if ( ! empty( $template_global ) ) {
...
if ( ! function_exists( 'gulir_get_author_page_settings' ) ) {
	function gulir_get_author_page_settings() {

		$author_id = get_queried_object_id();

		$settings                         = gulir_get_archive_page_settings( 'author_' );
		$settings['uuid']                 = 'uid_author_' . $author_id;
		$settings['follow_author_header'] = gulir_get_option( 'follow_author_header' );
...
if ( ! function_exists( 'gulir_get_search_page_settings' ) ) {
	function gulir_get_search_page_settings() {

		$settings                    = gulir_get_archive_page_settings( 'search_' );
		$settings['search_header']   = gulir_get_option( 'search_header' );
		$settings['header_scheme']   = gulir_get_option( 'search_header_scheme' );
		$settings['s']               = get_search_query( 's' );
		$settings['posts_per_page']  = gulir_get_option( 'search_posts_per_page' );
		$settings['template_global'] = gulir_get_option( 'search_template_global' );
		$settings['top_template']    = gulir_get_option( 'search_top_template' );
...
if ( ! function_exists( 'gulir_get_tag_page_settings' ) ) {
	function gulir_get_tag_page_settings() {

		$settings = gulir_get_archive_page_settings();
		$tag_id   = get_queried_object_id();
		$data     = rb_get_term_meta( 'gulir_category_meta', $tag_id );

		$tag_header       = ! empty( $data['archive_header'] ) ? $data['archive_header'] : gulir_get_option( 'tag_header' );
		$template_global  = ! empty( $data['template_global'] ) ? $data['template_global'] : gulir_get_option( 'tag_template_global' );
		$posts_per_page   = ! empty( $data['posts_per_page'] ) ? $data['posts_per_page'] : gulir_get_option( 'tag_posts_per_page' );
		$template         = ! empty( $data['template'] ) ? $data['template'] : gulir_get_option( 'tag_template' );
		$template_display = ! empty( $data['template_display'] ) ? $data['template_display'] : gulir_get_option( 'tag_template_display' );
...
if ( ! function_exists( 'gulir_get_page_header_style' ) ) {
	function gulir_get_page_header_style( $page_id = '' ) {
		...
		if ( empty( $setting ) || 'default' === $setting ) {
			$setting = gulir_get_option( 'page_page_header_style' );
		}
...
if ( ! function_exists( 'gulir_get_page_content_width' ) ) {
	/**
	 * @param string $page_id
	 *
	 * @return false|mixed|void
	 */
	function gulir_get_page_content_width( $page_id = '' ) {
		...
		if ( empty( $setting ) || 'default' === $setting ) {
			$setting = gulir_get_option( 'page_width_wo_sb' );
...
if ( ! function_exists( 'gulir_is_sw_header' ) ) {
	/**
	 * @param string $page_id
	 *
	 * @return false|mixed|void
	 */
	function gulir_is_sw_header( $page_id = '' ) {
		...
		if ( empty( $setting ) || 'default' === $setting ) {
			$setting = gulir_get_option( 'page_header_width' );
		}
 ...
 * @param array $settings
 */
if ( ! function_exists( 'gulir_carousel_footer' ) ) {
	function gulir_carousel_footer( $settings = [] ) {
 ...
 * @param array $settings
 */
if ( ! function_exists( 'gulir_carousel_attrs' ) ) {
	function gulir_carousel_attrs( $settings = [] ) {
 ...
 * @param array $settings
 */
if ( ! function_exists( 'gulir_slider_attrs' ) ) {
	function gulir_slider_attrs( $settings = [] ) {
...
if ( ! function_exists( 'gulir_get_single_crop_size' ) ) {
	/**
	 * @param string $default
	 *
	 * @return array|false|mixed|string|void
	 */
	function gulir_get_single_crop_size( $default = 'full' ) {

		$crop_size = rb_get_meta( 'featured_crop_size' );
		if ( empty( $crop_size ) ) {
			$crop_size = gulir_get_option( 'single_crop_size' );
		}
...
if ( ! function_exists( 'gulir_clear_settings_assets' ) ) {
	/**
	 * @param array $settings
	 *
	 * @return array|mixed
	 ...
	 */
	function gulir_clear_settings_assets( $settings = [] ) {

		$default = apply_filters( 'gulir_ajax_params', [
				'uuid'               => '',
				'category'           => '',
				'name'               => '',
				'categories'         => '',
...
if ( ! function_exists( 'gulir_clear_settings_query' ) ) {
	/**
	 * @param array $settings
	 *
	 * @return array|mixed
	 ...
	 */
	function gulir_clear_settings_query( $settings = [] ) {
...
if ( ! function_exists( 'gulir_live_block_localize' ) ) {
	/**
	 * @param $settings
	 */
	function gulir_live_block_localize( $settings ) {

		if ( ! empty( $settings['content_source'] ) && 'recommended' === $settings['content_source'] ) {
			$GLOBALS['gulir_yes_recommended'] = true;
		}
...
if ( ! function_exists( 'gulir_is_category_style_none_bg' ) ) {
	/**
	 * @param string $style
	 *
	 * @return bool
	 */
	function gulir_is_category_style_none_bg( $style = '' ) {
...
if ( ! function_exists( 'gulir_extra_meta_labels' ) ) {
	function gulir_extra_meta_labels( $settings = [] ) {
...
if ( ! function_exists( 'gulir_update_word_count' ) ) {
	function gulir_update_word_count( $post_id = '' ) {
		...
		$count   = gulir_count_content( $content );
		update_post_meta( $post_id, 'gulir_content_total_word', $count );

		return $count;
	}
}

if ( ! function_exists( 'gulir_is_author_tick' ) ) {
	function gulir_is_author_tick( $author_id = false ) {
		...
		if ( user_can( (int) $author_id, 'edit_posts' ) ) {
			return gulir_get_option( 'author_tick', false );
		}
...
if ( ! function_exists( 'gulir_get_svg_content' ) ) {
	/**
	 * @param string $attachment_id
	 *
	 * @return bool|mixed|string
	 */
	function gulir_get_svg_content( $attachment_id = '' ) {
