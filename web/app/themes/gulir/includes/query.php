...
if ( ! function_exists( 'gulir_get_order_query_params' ) ) {
	function gulir_get_order_query_params( $params, $order ) {
		...
		switch ( $order ) {
			...
			case 'popular_y':
				...
				break;
			case 'top_review' :
				$params['meta_key']  = 'gulir_review_average';
			...
			case 'last_review' :
				...
				break;
			case 'sponsored' :
				$params['meta_key'] = 'gulir_sponsored';
				$params['orderby']  = 'date';
				$params['order']    = 'DESC';
				break;
			case 'rand':
...
if ( ! function_exists( 'gulir_query' ) ) {
	function gulir_query( $data = [], $paged = null ) {

		if ( ! empty( $data['query_mode'] ) && 'global' === $data['query_mode'] && ! gulir_is_template_preview() ) {
			...
			if ( empty( $wp_query->is_singular ) ) {
				...
				$_query = $wp_query;
				if ( ! empty( $data['unique'] ) && ! empty( $GLOBALS['gulir_queried_ids'] ) && is_array( $GLOBALS['gulir_queried_ids'] ) ) {
		...
		if ( ! empty( $data['post_in'] ) ) {
			...
			if ( isset( $GLOBALS['gulir_queried_ids'] ) && count( $GLOBALS['gulir_queried_ids'] ) && ! empty( $data['unique'] ) ) {
				$excluded_ids = array_merge( $excluded_ids, $GLOBALS['gulir_queried_ids'] );
			}
		...
		$params = gulir_get_order_query_params( $params, $data['order'] );

		$_query = new WP_Query( $params );
		if ( ! empty( $GLOBALS['gulir_queried_ids'] ) && is_array( $GLOBALS['gulir_queried_ids'] ) ) {
			$_query->set( 'gulir_queried_ids', $GLOBALS['gulir_queried_ids'] );
		}
		gulir_add_queried_ids( $_query );
		do_action( 'gulir_after_query', $_query, $data );

		return $_query;
	}
}

if ( ! function_exists( 'gulir_query_related' ) ) {
	/**
	 * @param array $data
	 * @param int   $paged
	 *
	 * @return WP_Query
	 */
	function gulir_query_related( $data = [], $paged = 1 ) {
		...
		if ( empty( $data['where'] ) ) {
			$data['where'] = gulir_get_option( 'single_post_related_where', 'all' );
		}

		if ( empty( $data['posts_per_page'] ) ) {
			$data['posts_per_page'] = gulir_get_option( 'single_post_related_total' );
		}

		if ( empty( $data['orderby'] ) ) {
			$data['orderby'] = gulir_get_option( 'single_post_related_order' );
			if ( empty( $data['orderby'] ) ) {
				$data['orderby'] = 'rand';
			}
		}
		...
		if ( ! empty( $data['post_in'] ) ) {
			...
			if ( isset( $GLOBALS['gulir_queried_ids'] ) && count( $GLOBALS['gulir_queried_ids'] ) ) {
				$excluded_ids = array_merge( $excluded_ids, $GLOBALS['gulir_queried_ids'] );
			}
			if ( is_array( $excluded_ids ) ) {
				$params['post__not_in'] = array_unique( $excluded_ids );
...
if ( ! function_exists( 'gulir_personalize_query' ) ) {
	/**
	 * @param $settings
	 *
	 * @return false|mixed|WP_Query|null
	 */
	function gulir_personalize_query( $settings ) {

		if ( ! empty( $settings['content_source'] ) ) {
			if ( 'saved' === $settings['content_source'] ) {
				return Gulir_Personalize::get_instance()->saved_posts_query( $settings );
			} elseif ( 'history' === $settings['content_source'] ) {
				return Gulir_Personalize::get_instance()->reading_history_query( $settings );
			}
		}

		return Gulir_Personalize::get_instance()->recommended_query( $settings );
	}
}

if ( ! function_exists( 'gulir_add_queried_ids' ) ) {
	function gulir_add_queried_ids( $_query ) {
		...
		if ( ! empty( $_query->posts ) ) {
			...
			if ( is_array( $post_ids ) ) {
				$GLOBALS['gulir_queried_ids'] = array_unique( array_merge( $GLOBALS['gulir_queried_ids'], $post_ids ) );
			}
		}
	}
}