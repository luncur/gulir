...
add_action( 'after_setup_theme', [ 'Gulir_Register_Options', 'get_instance' ], 0 );

if ( ! class_exists( 'Gulir_Register_Options' ) ) {
	class Gulir_Register_Options {
		...
		public function __construct() {
			...
			$this->load_files();
			Redux::setArgs( GULIR_TOS_ID, $this->get_params() );
		...
		function load_files() {
			foreach ( $this->sources as $name ) {
				require_once gulir_get_file_path( 'backend/panels/' . $name . '.php' );
			}
		}

		public function display_name() {

			return esc_html__( 'Gulir Panel', 'gulir' ) . '<span class="p-version">' . GULIR_THEME_VERSION . '</span>';
		}

		public function display_version() {

			return '<span class="pdocs-info"><i class="el el-folder-open"></i><a href="https://help.luncur.com/gulir" target="_blank">' . esc_html__( 'Online Documentation', 'gulir' ) . '</a></span>';
		}
		...
		public function get_params() {
			...
			return [
				'opt_name'                  => GULIR_TOS_ID,
				'display_name'              => $this->display_name(),
				'display_version'           => $this->display_version(),
				'allow_sub_menu'            => true,
				'menu_title'                => esc_html__( 'Theme Options', 'gulir' ),
				'page_title'                => esc_html__( 'Theme Options', 'gulir' ),
				'menu_type'                 => 'submenu',
				'page_parent'               => 'gulir-admin',
		...
		public function register_options() {
				...
				foreach ( $this->funcs_name as $name ) {
					$func = 'gulir_register_options_' . $name;
					if ( function_exists( $func ) ) {
						Redux::setSection( GULIR_TOS_ID, call_user_func( $func ) );
					}
