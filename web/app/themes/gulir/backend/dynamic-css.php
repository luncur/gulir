...
if ( ! class_exists( 'Gulir_Css' ) ) {
	class Gulir_Css {
		...
		public function __construct() {
			...
			add_action( 'update_option_' . GULIR_TOS_ID, [ $this, 'update_hook' ], 10, 2 );
		...
		function write_css( $settings = [] ) {
			...
			update_option( 'gulir_style_cache', addslashes( $output ) );
		 ...
		 * then saves the generated CSS string into the `gulir_term_style_cache` option in the database.
		...
		public function write_term_css() {
			...
			update_option( 'gulir_term_style_cache', addslashes( $output ) );
		 ...
		 * @param $handle
		 */
		public function load( $handle = 'gulir-style' ) {

			$output      = get_option( 'gulir_style_cache' );
			$term_output = get_option( 'gulir_term_style_cache' );
		...
		public function font_var( $settings = [], $prefix = '', $config_id = '' ) {
			...
			if ( ! empty( $settings[ $config_id ]['font-family'] ) ) {
				if ( gulir_get_option( 'disable_google_font' ) ) {
		...
		function get_glass_background_var( $settings = [], $name = '', $config_id = '', $dark = false ) {
		 ...
		 * @return string
		 */
		function get_sub_menu_var_data_dark( $settings = [] ) {
		 ...
		 * fetches their custom metadata stored under `gulir_category_meta`,
		...
		public function term_output() {
			...
			foreach ( $term_ids as $id ) {
				$values = rb_get_term_meta( 'gulir_category_meta', $id );
		...
		function get_dynamic_style( $settings = [] ) {
			...
			if ( ! empty( $settings['header_search_custom_icon']['url'] ) ) {
				if ( gulir_is_svg( $settings['header_search_custom_icon']['url'] ) ) {
			...
			if ( ! empty( $settings['cart_custom_icon']['url'] ) ) {
				if ( gulir_is_svg( $settings['cart_custom_icon']['url'] ) ) {
			...
			if ( ! empty( $settings['notification_custom_icon']['url'] ) ) {
				if ( gulir_is_svg( $settings['notification_custom_icon']['url'] ) ) {
			...
			if ( ! empty( $settings['login_custom_icon']['url'] ) ) {
				if ( gulir_is_svg( $settings['login_custom_icon']['url'] ) ) {
				...
				foreach ( $menus as $menu ) {
					...
					if ( is_array( $nav_items ) ) {
						foreach ( $nav_items as $item_id ) {
							...
							$item_data = rb_get_nav_item_meta( 'gulir_menu_meta', $item_id );
			...
			if ( ! empty( $settings['podcast_custom_icon']['url'] ) ) {
				if ( gulir_is_svg( $settings['podcast_custom_icon']['url'] ) ) {
